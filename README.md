...
└── figures/                              # 5 类骨架图的独立 SVG，可复用到 PPT
```

报告固定五段：**摘要 → 行业背景 → 商业模式 → 业务流程 → 术语表**，内嵌 5 类骨架图（发展时间轴、产品服务全景、业务协作链、资金流转、L1 流程）。
...
| 产物     | Markdown                         | 杂志级单文件 HTML + PDF + 独立 SVG                           |
